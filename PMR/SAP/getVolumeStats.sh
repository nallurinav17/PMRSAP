...
for dc in ${allDC};
do 
$SSH $STANDBY "$HDFSCMD  /data/output/$dc/Midm/$myyr/$mymo/$myda/GVS.MIDM_Aggregate*" | awk '{print $5$8}'| awk -v "t1=$TIMESTAMP" -v "dc=$dc" -F "/" 'BEGIN{sum1=0} {sum1+= $1} END{printf "%s, SAP, %s, MIDM_aggregated_data_DC_volume,%s\n", t1, dc, sum1}';
done 2>/dev/null | tee -a ${TMPFILE}.midmagg

# MIDM_aggregated_data_total_volume
awk -v "t1=$TIMESTAMP" -F "," 'BEGIN{sum1=0} {sum+= $NF} END{printf "%s, SAP, MIDM, MIDM_aggregated_data_total_volume, %s\n", t1, sum}' ${TMPFILE}.midmagg 
# Cleanup
...
for dc in ${midmDC};
do
$SSH $STANDBY "$HDFSCMD  /data/output/$dc/Midm/$myyr/$mymo/$myda/GVS.MIDM_Enriched*" | awk '{print $5$8}'| awk -v "t1=$TIMESTAMP" -v "dc=$dc" -F "/" 'BEGIN{sum1=0} {sum1+= $1} END{printf "%s, SAP, %s, MIDM_enriched_data_DC_volume ,%s\n", t1, dc, sum1}';
done 2>/dev/null | tee -a ${TMPFILE}.midmenr

# MIDM_enriched_data_total_volume
awk -v "t1=$TIMESTAMP" -F "," 'BEGIN{sum1=0} {sum+= $NF} END{printf "%s, SAP, MIDM, MIDM_enriched_data_total_volume, %s\n", t1, sum}' ${TMPFILE}.midmenr 
# Cleanup
